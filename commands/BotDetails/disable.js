...
module.exports = {
    name: 'disable',
    description: 'Disables categories for use in the server.',
    ...
    async execute(message, args, client, prefix)  {
        ...
        if(GuildDoc[category.toLowerCase()] == false){
            return message.channel.send('That category is already `disabled`!')
        }
        else if(GuildDoc[category.toLowerCase()] == true){
            GuildDoc[category.toLowerCase()] = false
            await GuildDoc.save()
            ...
            .setColor('RED')
            .setDescription(`
            ${category} has been disabled! Remember to check ${prefix}help to see which commands are associated with this category.
            `)
            .setTimestamp()
